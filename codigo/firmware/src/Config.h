...
// =============================================================================
// ASIGNACIÓN DE PINES GPIO Y CANALES PCNT (ESP32)
// =============================================================================
// Cada motor tiene:
// - pwm: Pin de PWM directo del ESP32 (8 pines independientes).
// - dir_mcp: Bit de salida en el expansor I2C MCP23017 (0 a 7).
// - encA / encB: Pines de cuadratura mapeados a las unidades PCNT del ESP32.
struct MotorPins {
    uint8_t pwm;
    uint8_t dir_mcp; // Índice del bit en el MCP23017
    int encA;
    int encB;
    int pcnt_unit;   // Unidad PCNT asignada (0 a 7 en ESP32)
};

const MotorPins MOTOR_PINS[8] = {
    // Motor, PWM Pin, Bit MCP, encA, encB, Unidad PCNT
    { 2, 0, 34, 35, 0 },
    { 12, 1, 36, 39, 1 },
    { 14, 2, 32, 33, 2 },
    { 16, 3, 25, 26, 3 },
    ...
    { 18, 5, 23, 19, 5 },
    { 19, 6,  4, 15, 6 },
    { 25, 7,  2, 10, 7 }  // Reajustado para evitar superposiciones
};
